...
export default function SessionDetail() {
  ...
  // In real app, fetch session data based on sessionId
  ...
  const tabs = [
    ...
    { id: "assignments", label: "Assignment Submission", badge: course.assignments.length, color: "bg-gradient-to-r from-warning-500 to-warning-600 text-white" }
  ];
    ...
    <StudentLayout>
      <div className="max-w-full mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-sm text-neutral-900 font-medium">{session.title}</span>
        </div>
        ...
        <div className="flex items-center justify-between mb-8 p-6 border border-accent-200 rounded-xl bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-accent-600 text-lg">{session.topic}</p>
              <p className="text-neutral-500 mt-1">{session.topicTitle}</p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-gradient-to-br from-primary-50 to-primary-100 border border-primary-200 rounded-xl">
                  ...
                  <p className="text-primary-700 font-medium">{session.topicTitle}</p>
                </div>
                ...
                <div className="p-4 bg-gradient-to-br from-warning-50 to-warning-100 border border-warning-200 rounded-xl">
                  ...
                  <p className="text-warning-700 font-medium">{session.required ? 'Yes' : 'No'}</p>
                </div>
              </div>
              ...
              <div className="p-6 bg-gradient-to-br from-info-50 to-info-100 border border-info-200 rounded-xl">
                ...
                <div className="p-4 bg-white border border-info-200 rounded-lg">
                  <p className="text-info-700 leading-relaxed text-base">{session.contentSummary}</p>
                </div>
              </div>
              
              {/* Pre-Reading Material */}
              {session.preReadingUrl && (
                ...
                <div key={assignment.id} className="p-4 border border-accent-200 rounded-lg bg-white hover:bg-accent-25 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                      assignment.status === "graded" ? "bg-success-500 text-white" :
                      assignment.status === "submitted" ? "bg-accent-500 text-white" :
                      assignment.status === "pending" ? "bg-warning-500 text-white" :
                  ...
                  {assignment.instructorRemarks && (
                    <div className="mb-4 p-3 bg-info-50 border border-info-200 rounded-lg">
                      <h5 className="text-sm font-semibold text-info-800 mb-2">Instructor Remarks:</h5>
                      <p className="text-sm text-info-700 leading-relaxed">{assignment.instructorRemarks}</p>
                    </div>
                  )}
